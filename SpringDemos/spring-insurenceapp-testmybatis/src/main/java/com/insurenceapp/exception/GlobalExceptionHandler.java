package com.insurenceapp.exception;
...
import com.insurenceapp.model.ApiErrors;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler{
	...
	protected ResponseEntity<Object> handleMissingServletRequestParameter(MissingServletRequestParameterException ex,
		...
		HttpStatus statusMessage=HttpStatus.valueOf(status.value());
		ApiErrors errors=new ApiErrors(timestamp, status.value() , ex.getMessage(),statusMessage);
		headers.add("error", "missing request parameter");
		return ResponseEntity.status(status).headers(headers).body(errors);
	}
	...
	@ExceptionHandler(InsurenceNotFoundException.class)
	public ResponseEntity<Object> handleInsurenceNotFound(InsurenceNotFoundException ex){
		LocalDateTime timestamp=LocalDateTime.now();
